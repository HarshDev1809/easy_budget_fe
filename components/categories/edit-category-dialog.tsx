...
import { parseCronString } from "@/lib/utils"
...
function getInitialCustomValues(category: Category) {
  const cron = category.customCron || category.renewCron;
...
export function EditCategoryDialog({ category, onSuccess }: EditCategoryDialogProps) {
  ...
  const initialCustom = getInitialCustomValues(category)
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      renewCycle: category.renewCycle || "monthly",
      renewDayOfWeek: category.renewDayOfWeek !== undefined ? category.renewDayOfWeek : 1,
      renewDayOfMonth: category.renewDayOfMonth !== undefined ? category.renewDayOfMonth : 1,
      customType: initialCustom.customType,
      customTime: initialCustom.customTime,
      customDaysOfMonth: initialCustom.customDaysOfMonth,
      customDaysOfWeek: initialCustom.customDaysOfWeek,
    },
  })

  // Reset form when dialog opens or category updates
  React.useEffect(() => {
    if (open) {
      const currentCustom = getInitialCustomValues(category);
      form.reset({
        name: category.name,
        baseAmount: category.baseAmount,
        carryForward: category.carryForward,
        ...
        renewDayOfWeek: category.renewDayOfWeek !== undefined ? category.renewDayOfWeek : 1,
        renewDayOfMonth: category.renewDayOfMonth !== undefined ? category.renewDayOfMonth : 1,
        customType: currentCustom.customType,
        customTime: currentCustom.customTime,
        customDaysOfMonth: currentCustom.customDaysOfMonth,
        customDaysOfWeek: currentCustom.customDaysOfWeek,
      })
    }
  }, [open, category, form])
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      await apiClient(`/api/v1/categories/${category.id}`, {
        method: "PATCH",
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Modify your budget category configurations.
          </DialogDescription>
        </DialogHeader>
