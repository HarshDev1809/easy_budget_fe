...
import { Book, Category, Transaction, ApiResponse } from "@/lib/types"
...
export function EditTransactionDialog({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      createdAt: formatToLocalDatetime(new Date(transaction.createdAt)),
      paidAt: transaction.paidAt ? formatToLocalDatetime(new Date(transaction.paidAt)) : formatToLocalDatetime(new Date()),
    },
  })
  ...
  // Fetch categories for the selected book
  ...
  }, [selectedBookId, fetchCategories])

  // Handle open state reset to initial transaction values
  React.useEffect(() => {
    if (open) {
      form.reset({
        name: transaction.name,
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      await apiClient(`/api/v1/transactions/${transaction.id}`, {
        method: "PUT",
